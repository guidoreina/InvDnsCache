...
static void PrintCache(dns_cache_t* ip_cache, SIZE_T ip_size);
static void PrintIP(const UINT8* ip, SIZE_T ip_size);
...
void PrintDnsCache()
{
  PrintCache(&ipv4_cache, 4);
  PrintCache(&ipv6_cache, 16);
}
...
void PrintCache(dns_cache_t* ip_cache, SIZE_T ip_size)
{
  ...
  for (i = 0; i < ip_cache->nbuckets; i++) {
    printf("\tBucket %u:\n", i);

    header = &ip_cache->buckets[i];
    entry = (cache_entry_t*) header->next;

    while (entry != (cache_entry_t*) header) {
      printf("\t\t");
      PrintIP(entry->ip, ip_size);
      printf("\n");
  ...
  entry = (cache_entry_t*) ip_cache->time.newer;

  while (entry != (cache_entry_t*) &ip_cache->time) {
    printf("\t\t");
    PrintIP(entry->ip, ip_size);
    printf("\n");
  ...
  for (i = 0; i < ip_cache->nbuckets; i++) {
    ...
    if (count != 1) {
      printf("\t\tBucket %u: %u IP addresses.\n", i, count);
    } else {
      printf("\t\tBucket %u: %u IP address.\n", i, count);
    }
#else
    printf("\t\tBucket %u: %zu IP address%s.\n",
           i,
           count,
           (count != 1) ? "es" : "");
#endif
...
void PrintIP(const UINT8* ip, SIZE_T ip_size)
{
  if (ip_size == 4) {
